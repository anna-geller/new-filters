...
export default function KvStoreTable({ rows, columns }: KvStoreTableProps) {
  const visibleColumns = useMemo(
    () => columns.filter((column) => column.visible).sort((a, b) => a.order - b.order),
    [columns],
  );

  const renderCell = (row: KvStoreRow, columnId: string) => {
    switch (columnId) {
      case "namespace":
      ...
      case "lastModified":
        return <span className="text-foreground/90 font-mono text-xs md:text-sm">{row.lastModified}</span>;
      case "expirationDate":
        return row.expirationDate ? (
          <span className="text-foreground/90 font-mono text-xs md:text-sm">{row.expirationDate}</span>
        ) : (
          <span className="text-muted-foreground">—</span>
        );
      default:
