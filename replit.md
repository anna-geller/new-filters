...
- **Flow Inputs Display**: Added Flow Inputs section to InputsPanel displaying all Input nodes from the flow at the top of the panel, before Task Outputs and Execution Context. Input nodes show their ID, type, description, and Kestra expression (`{{ inputs.inputId }}`), and support drag-and-drop and copy operations for easy reference in task properties.
- **Output Node All-Tasks Input Display**: Modified InputsPanel to show all task outputs from the entire flow when editing Output nodes (via `showAllTasks` prop). Regular task nodes continue to see only connected upstream task outputs, maintaining data flow clarity. This enables Output nodes to reference any task output in the flow when defining flow output schemas.
- **Output Node Properties Implementation**: Added complete properties panel for Output nodes with 8 configurable fields: Type (dropdown: STRING/NUMBER/BOOLEAN/OBJECT/ARRAY), ID, Display Name, Description, Validator, Prefill, Value, and Required toggle. Properties panel supports drag-and-drop of Kestra expressions (e.g., `{{ outputs.taskId.value }}`) into Value, Description, Validator, and Prefill fields. Output nodes use `__output_node__` metadata entry in taskMetadata.ts to drive UI generation. Run button enabled for Output nodes to execute upstream tasks needed to materialize flow outputs. Auto-save mechanism persists config changes immediately on every field edit by calling `onSave` with latest `node.data.config`.
